...
with torch.no_grad():
    ...
    net.net.load_state_dict(torch.load("../Burgers/checkpoint/"+elementType+"_weight_clean25.pt",weights_only=False,map_location=device))
    net.net = net.net.to(device)
    u_pred = net.net(torch.hstack((x_ts, t_ts))).cpu().numpy().reshape((100, 256)).T
...
# l1 = net.net.state_dict()["lambda_1"].cpu().item()
# l2 = np.exp(net.net.state_dict()["lambda_2"].cpu().item())
# net.net.load_state_dict(torch.load("../Burgers/checkpoint/"+elementType+"_weight_clean.pt",weights_only=False,map_location='cuda:0'))
# l1_noisy = net.net.state_dict()["lambda_1"].cpu().item()
# l2_noisy = net.net.state_dict()["lambda_2"].cpu().item()
#
...
fig.savefig(
    "../Burgers/res/"+elementType+"_solution25.pdf", bbox_inches="tight", pad_inches=0, dpi=500,
)
plt.show(block=True)